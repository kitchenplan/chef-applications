case node["platform_family"]
    when 'mac_os_x'
        ...
        unless File.exists?(gtemplate)
            directory "#{node['etc']['passwd'][node['current_user']]['dir']}/Library/Application Support/OmniGraffle/Templates" do
        ...
        gdiagramstyle = "#{node['etc']['passwd'][node['current_user']]['dir']}/Library/Application Support/OmniGraffle/Diagram Styles/Konigi.gdiagramstyle"
        unless File.exists?(gdiagramstyle)
            directory "#{node['etc']['passwd'][node['current_user']]['dir']}/Library/Application Support/OmniGraffle/Diagram Styles" do
                owner node['current_user']
                mode 0755
                action :create
                recursive true
        ...
        stencils = "#{node['etc']['passwd'][node['current_user']]['dir']}/Library/Application Support/OmniGraffle/Stencils/Konigi Wireframe Stencils v3"
        unless File.exists?(stencils)
            directory "#{node['etc']['passwd'][node['current_user']]['dir']}/Library/Application Support/OmniGraffle/Stencils" do
                owner node['current_user']
                mode 0755
                action :create
                recursive true
